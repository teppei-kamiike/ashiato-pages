...
1. 当方は，本アプリへのアクセス状況やそのご利用方法に関する情報を収集することがあります。これには以下の情報が含まれます。
    - 本アプリでは、メッセージ生成ツールとしてChat Completions API(OpenAI Inc.)を利用しています。ユーザが本アプリ内に保存したデータの一部をChat Completions APIに渡すことでメッセージを生成しています。OpenAI社での個人情報の取り扱いについては以下のOpenAIプライバシーポリシーのリンクよりご確認ください。
